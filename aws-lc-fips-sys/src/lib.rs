...
// When no-build is active (e.g. via --all-features), the FIPS C library is not
// compiled and no bindings are generated. The crate is intentionally empty;
// all FIPS code paths in aws-lc-rs are gated behind
// cfg(all(feature = "fips", not(feature = "non-fips"))) and are dead under
// --all-features.
